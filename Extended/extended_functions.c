...
board_struct *make_a_move(int row_coordinate, int col_coordinate, board_struct *player_board, board_struct *current_board, 
						  success_failure *success_index, full_empty hashing_array[VERY_LARGE_NUMBER])
{

	board_struct *uninvestigated_board;

	current_board = check_board_contents(current_board, player_board, success_index, row_coordinate, col_coordinate, hashing_array);
	...
	if(*success_index == successful || *success_index == symmetric_successful){
		...
		current_board = make_a_move(row_coordinate, col_coordinate, uninvestigated_board, current_board, success_index, hashing_array);
	}
...
//end of the list. Also in the extended version we make the (fair) assumption we never want to move away from the desired
//location.
		...
		for(j = BORDER; j < BOARD_W; ++j){
			if(player_board->board_array[i][j] == peg && *success_index == unsuccessful){
				...
				current_board = attempt_to_move(player_board, current_board, success_index, row_coordinate, col_coordinate, 
												hashing_array, i, j, 1, 0);

				//Move left.
				current_board = attempt_to_move(player_board, current_board, success_index, row_coordinate, col_coordinate, 
												hashing_array, i, j, 0, -1);
...
//In the extended version we incorporate hashing. The imperfect hash as described in the documentation generates a (semi) unique
//hashing number for each board. These are then modulused by a large number and that index in an array is used to check if we may
//have a repeated board. If this shows repetition we now compare all of the hash numbers, going backwards through the list until
//we reach a potential repetition, when this occurs we check the symmetric and regular boards to confirm there has been an identical
//board before.
...
//Checks to see if the current board has a peg at the solution space. It checks both the regular and symmetric
//boards.
...
unsigned long long generate_first_half(board_struct *new_board)
{
	...
	for(i = BORDER; i < BOARD_H - BORDER; ++i){
		...
		power_of_ten *= 10; 											//Used to shift the digit up by one place in the num each time.
		counter = 0;
	}
...
{
	int i, j, temp; 
	unsigned long long counter = 0, symmetric_counter = 0;
	
	for(j = BORDER; j < BOARD_W; ++j){
		for(i = BORDER; i < BOARD_H - BORDER; ++i){

			temp = BOARD_W - BORDER - j;									//Used to count cols from the other side.
		...
		current_power_of_ten *= 10;										//Used to shift the digit up by one place in the num each time.
		counter = 0;
		symmetric_counter = 0;
	}
}
...
contained_indicator check_unique_hash_number(board_struct *new_board, board_struct *current_board, int last_peg_number)
{
	...
	if(current_board != NULL){
		...
		if(current_board->number_pegs == last_peg_number && indicator == unseen){
...
//one showing the movement and one showing the position afterwards. The counter is used to make sure that
//the first board is not treated as if it has come from another move.
...
//Also in the extended version if the symmetric solution has been achieved then it flips the whole board over 
...
//different colour dependent. The benefit of not having the colours in the struct is that you
//can easily mix them to create new ones.
